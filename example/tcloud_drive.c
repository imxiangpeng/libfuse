...
static int tcloud_drive_readdir(const char *path, void *dbuf, fuse_fill_dir_t filler,
...
static int tcloud_drive_releasedir(const char *path, struct fuse_file_info *fi)
{
    printf("%s(%d): ........\n", __FUNCTION__, __LINE__);
    return 0;
}
...
static int tcloud_drive_rename(const char *from, const char *to, unsigned int flags)
{
...
static int tcloud_drive_read(const char *path, char *rbuf, size_t size, off_t offset,
                      struct fuse_file_info *fi)
{
    printf("%s(%d): ........\n", __FUNCTION__, __LINE__);
    return 0;
